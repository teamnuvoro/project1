...
export const sessions = pgTable("sessions", {
  ...
  sessionContext: jsonb("session_context"), // {topics: [], mood: string, summary: string}
...
// Call Status enum for Vapi call sessions
export const callStatusEnum = pgEnum("call_status", ["started", "in_progress", "completed", "failed", "aborted"]);

// Vapi Call Sessions table - tracks Vapi-specific call data
export const callSessions = pgTable("call_sessions", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  vapiCallId: text("vapi_call_id").unique(),
  status: callStatusEnum("status").notNull().default("started"),
  startedAt: timestamp("started_at", { withTimezone: true }).notNull().defaultNow(),
  endedAt: timestamp("ended_at", { withTimezone: true }),
  durationSeconds: integer("duration_seconds"),
  transcript: text("transcript"),
...
export const usageStats = pgTable("usage_stats", {
  ...
  callDurationSeconds: integer("call_duration_seconds").notNull().default(0),
