...
// Get chat sessions with message previews
router.get('/api/sessions', async (req: Request, res: Response) => {
  try {
    ...
    // Fetch message counts and previews for each session
    ...
    // Group messages by session and create enriched session data
    const enrichedSessions = sessions.map(session => {
      ...
      const firstAiMsg = sessionMessages.find(m => m.role === 'ai' || m.role === 'assistant');
      
      return {
        ...session,
        messageCount,
        preview: {
          userMessage: firstUserMsg?.text?.substring(0, 100) || null,
          aiResponse: firstAiMsg?.text?.substring(0, 100) || null,
        }
      };
    });
...
router.get('/api/sessions/:sessionId', async (req: Request, res: Response) => {
  try {
    ...
    if (sessionError) {
      ...
      console.error('[/api/sessions/:id] Session error:', sessionError);
    ...
    if (messagesError) {
      console.error('[/api/sessions/:id] Messages error:', messagesError);
    }
