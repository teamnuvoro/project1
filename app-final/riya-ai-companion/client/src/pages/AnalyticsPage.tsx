...
const defaultAnalytics: AnalyticsResponse = {
  engagement: { totalUsers: 1, activeUsers7d: 1, avgMessagesPerSession: 0, totalMessages: 0, voiceCallSessions: 0, voiceMinutes: 0 },
  conversion: { premiumUsers: 0, freeToPaidConversion: 0, planBreakdown: {} },
  quality: { confidenceScore: 75 }
};
...
export default function AnalyticsPage() {
  ...
  const confidenceScore = data?.quality?.confidenceScore ?? 75;
  const totalMessages = data?.engagement?.totalMessages ?? 0;
  const voiceMinutes = data?.engagement?.voiceMinutes ?? 0;

  const strengthCards: StrengthCard[] = [
    {
      icon: <MessageCircle className="w-6 h-6" />,
      title: "Communication Style",
      score: Math.round(Math.min(100, Math.max(50, 70 + (totalMessages > 50 ? 15 : totalMessages / 4)))),
      color: "bg-blue-500",
      traits: [
    ...
    },
    {
      ...
      score: Math.round(Math.min(100, Math.max(50, 65 + (voiceMinutes > 30 ? 13 : voiceMinutes / 3)))),
      color: "bg-pink-500",
      traits: [
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <div className="animate-pulse text-muted-foreground">Loading your analytics...</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <button
            onClick={() => refetch()}
            className="px-4 py-2 gradient-primary-button text-white rounded-lg"
