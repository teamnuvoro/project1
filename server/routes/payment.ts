...
router.post('/api/payment/create-order', async (req: Request, res: Response) => {
  try {
    ...
    // Safety Check: Detect Mock IDs (generated by helper in Sandbox mode)
    // Mock IDs in cashfree.ts are "session_" + Date.now()
    if (sessionId.startsWith("session_") && sessionId.length < 30) {
      // Real session IDs are usually long UUID-like or base64.
      // Mock ID is just "session_178..." (approx 21 chars).
      // Let's rely on the environment check instead effectively
      if (process.env.CASHFREE_ENV !== 'TEST' && !process.env.CASHFREE_APP_ID?.startsWith('TEST')) {
        // If we think we are in Prod, but got a simple session ID, it might be mock?
        // Actually, safer to just check the Mode.
      }
    }
